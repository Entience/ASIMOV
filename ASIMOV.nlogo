...
globals [Addiction_Cycle_Phase switch_time stop_time]
...
Cslugs-own [ Somatic_Map App_State App_State_Switch ExpReward_pos ExpReward_neg Incentive Nutrition Satiation speed turn-angle
             ...
             R R_hermi R_flab R_drug RewardState deg_sensitization IN M M0 W1 W2 W3 dW3 W4 W5 RewardState_history
            ]
...
;    set h-suppression 0.99 * h-suppression
    ...
    ;set pain-switch 2 / (1 + e ^ (- 7 * (RewardState - pain + 2.1))) - 1;
  ...
  ;=================================================================;
   ...
   ;; sets somatic map variable, transforms sensory input into a "place code"
    ...
    ;set App_State 0.01 + (1 / (1 + exp(- (0.75 * Incentive - 9 * satiation  - 1.8 * pain-switch * (RewardState - Pain) ))) + 0.1 * ((App_State_Switch - 1) * 0.5)); + 0.25
    set App_State 0.01 + (1 / (1 + exp(- (0.75 * Incentive - 9 * satiation -  1.8 * Pain - 1.8 * pain-switch * RewardState))) + 0.1 * ((App_State_Switch - 1) * 0.5)); + 0.25
  ...
  ;=================================================================;
    ...
    ifelse any? drugtarget [
      ...
      [set delta_Vd alpha_drug * beta_drug * (0 - Vd)]
...
to update-RewardState
  ...
  ifelse Fix-var2: [set RewardState Fix-RewardExp][set RewardState -15 + 30 / (1 + e ^ (-(M - M0)))]
  set RewardState_history lput RewardState RewardState_history
...
;                      and Reward State can be fixed via the Fix-var2 switch and fix-RewardState slider.
;-------------------------------------------------------------------------------------------------------
    ...
    to Present
      if Presentation-Mode = true[
        ...
        if Presentation-Choice = "Drug vs Hermi"[
          create-drugs 1 [set odor_drug 0.5 set shape "circle" set size 1 set color yellow setxy (- xc) yc]
          create-hermis 1 [set odor_hermi 0.5 set odor_betaine 0.5 set shape "circle" set size 1 set color green + 2 setxy xc yc]
        ]
        if Presentation-Choice = "Drug vs Flab"[
          create-drugs 1 [set odor_drug 0.5 set shape "circle" set size 1 set color yellow setxy (- xc) yc]
          create-flabs 1 [set odor_flab 0.5 set odor_betaine 0.5 set shape "circle" set size 1 set color red + 2 setxy xc yc]
        ]
    ...
    to Feed
      if Presentation-Mode = true[
        ...
        if Feeding-Choice = "Odorless Drug"[
          create-drugs 1 [set odor_drug 0 set shape "circle" set size 1 set color yellow setxy xc yc]
        ]
...
;             Addiction Cycle
;=============================================================================================================
;                  The Addiction Cycle mode will start off a series of events that causes Cyberfiend to go
...
;
;                  Cyberfiend will start out in an environment with no drugs and only prey in the Drug-Free phase.
;                  In the Drug Introduced phase, 12 drugs are spawned, allowing Cyberfiend to start
;                  drug consumption. Note that drug consumption typically occurs when Cyberfiend is
...
;                  Cyberfiend is likely to undergo withdrawal and recovery from withdrawal.
;                  Following this is the Drug Without Reward phase, where 12 drug items are spawned again, emitting the same drug odor,
;                  but providing no reward instead of a high positive reward.
;                  In the beginning of this phase Cyberfiend is likely to demonstrate cravings by resuming drug consumption
...
"" ""
PENS
"pen-1" 1.0 0 -16777216 true "" "plot [RewardState] of Cslug 0;plot [ExpReward + ExpReward_neg] of Cslug 0"
...
"" ""
PENS
"default" 1.0 0 -5825686 true "" "plot [Incentive] of Cslug 0"
...
"" ""
PENS
"Drug" 1.0 0 -1184463 true "" "plot [drugcount] of Cslug 0"
"Hermi" 1.0 0 -13840069 true "" "plot [hermcount] of Cslug 0"
"Flab" 1.0 0 -2674135 true "" "plot [flabcount] of Cslug 0"
...
ASIMOV (Algorithm of Selectivity based on Incentive, Motivation and Optimized Valuation) is built onto the Cyberslug™ software, which reproduces the decisions for approach or avoidance in the predatory sea-slug _Pleurobranchaea californica_. It applies relations discovered in the nervous system of the real animal that underlie decisions in foraging for prey. The approach-avoidance decision is basic to foraging as well as most other economic behavior. ASIMOV adds specific algorithms for homeostatic plasticity in a neural circuit of reward state and for pain to recreate the hallmark processes of addiction: dependency, desensitization, withdrawal, resensitization and prolonged cravings. Fluctuations in reward state, caused by drug consumption and homeostatic plasticity, cause significant changes in nutritional intake and pain sensitivity throughout the addiction process.
...
Brown JW, Caetano-Anollés D, Catanho M, Gribkova E, Ryckman N, Tian K, Voloshin M, and Gillette R. Implementing Goal-Directed Foraging Decisions of a Simpler Nervous System in Simulation. In preparation, 2017.



## HOW IT WORKS

Approach-avoidance choice is organized around how the ASIMOV agent feels in terms of hunger, positive and negative rewards that it receives, the savory qualities of prey odor, pain sensation, and what it remembers about earlier experience with that prey. The agent adds up sensation, motivation (satiation/hunger and reward state), pain, and memory from moment-to-moment into its appetitive state. Appetitive state controls the switch for approach vs. avoidance turn responses to prey.

In ASIMOV, reward state integrates rewards from prey and drug consumption, and via homeostatic plasticity mechanisms, mediates habituation to rewarding cues, which is the basis of effects of drug desensitization and withdrawal. Reward state is in in reciprocal inhibition with pain, and both pain and reward state separately provide negative feedback to appetitive state.
...
As ASIMOV's predator consumes prey and drug, it learns to associate the odor of the specific item with the positive or negative reward that it receives. For instance, Hermis, pictured as green dots on the graphic interface, are valuable prey that give a positive reward when consumed, Flabs, pictured as red dots, are a noxious prey that give the predator a negative reward when consumed, while the drug, pictured as yellow dots, is an addictive substance that gives the predator a much larger positive reward. Therefore, through ongoing prey consumption, the agent can learn to associate Hermi odor and drug odor with positive reward and Flab odor with negative reward. These associations are established via Rescorla-Wagner learning rules, and play a part in setting the predator's appetitive state, which also relies on the predator's satiation and sensation of prey odor.
...
Drug effects on reward state alter sensitivity to pain. In drug withdrawal, which manifests as a negative reward state, the effects of pain and hunger are increased, causing ASIMOV's predator to show less selectivity towards prey of different values. In high reward states, which often immediately follow drug consumption, pain sensitivity is reduced and nutritional needs are ignored in favor of drug ingestion, leading to increased selectivity for high reward prey, and particularly for drug. While fluctuations in reward state contribute to increased drug consumption, the primary driver for high drug selectivity is the learned association between drug odor signature and high reward. This creates strong preference, or cravings, for the drug, so that even after recovery from withdrawal, cravings remain, and the predator unsurprisingly resumes drug consumption when available.
...
In the middle environment panel, the user can manipulate the positions of the predator and prey by clicking and dragging them around the environment. Everything, including the predator, and all prey and drugs, can also be immobilized by clicking on the Immobilize switch at the bottom left side. This option still allows for the agent's turning behavior, but not forward movement. There is a button at the bottom middle, which resets all variables to their default values.
...
Addiction Cycle Mode, when enabled, allows the user to observe ASIMOV's predator as it freely forages and experiences different phases of the addiction processes of desensitization, withdrawal, and cravings in a dynamic environment. In this mode the availability of drug changes over time, starting with an environment with only prey and no drug, and then adding and removing the drug, causing the predator to go through desensitization, withdrawal, and cravings. In the last phase, the drug is present with the drug odor signature but does not provide any reward on consumption. In this phase, drug consumption decreases significantly (see Fig. 6 for results from the Addiction Cycle Mode). The Addiction Cycle Mode lasts to 60000 software cycles (ticks), in which each phase lasts for 15000 ticks. During an initial “No Drug” phase, the environment contains only the prey Hermissenda and/or Flabellina, letting the predator forage and learn the corresponding associations. In the second “Drug Introduced” phase, the drug is introduced for the first time with a high reward on consumption. The third phase is “Drug Removed” and the fourth and last phase is “Drug without Reward”, where the drug is reintroduced with the same odor signature, but provides no reward on consumption, causing the predator to decrease its associative strength for the drug via a Rescorla-Wagner algorithm for extinction.
...
**Prey and Drug Population Controls**
The user can occupy the world with valuable and dangerous prey, Hermis and Flabs respectively, as well as a non-nutritious high-reward Drug with the Prey and Drug Population Control slider bars. In the beginning the simulation starts with 4 Drugs, 4 Hermis and 4 Flabs. ASIMOV's predator learns to prefer or avoid the different prey and may learn an increased preference towards the drug.

**Pain Application Controls**
Using the Pain Application Controls, the user can also apply a painful poke at the top-left or top-right of the predator via the Poke-Left or Poke-Right buttons. The severity of this painful stimulus can be controlled via the apply_pain slider.

**Fixation of Variables**
To observe the behavior of ASIMOV's predator when certain variables are held constant, the user can use the Fixation of Variables controls. The three variables that can be fixed are Satiation, Reward State, and Incentive. To fix the desired variable at a certain value, turn the corresponding Fix-var switch to ON, and set the corresponding slider to the desired value.

**Presentation Mode**
Presentation Mode is a useful tool to control the predator's food and drug intake, as well as to monitor its approach and avoidance behavior with certain prey and drug. Presentation Mode was used in combination with Fixation of variables to construct prey and drug response maps, as well as the pain response map as seen in the ASIMOV paper.
In Presentation Mode, ASIMOV's predator is immobilized, except for turning, and can be fed or
...
To present prey or drug, choose prey or drug type from the Presentation-Choice drop-down menu, then click on the Present button. This will spawn the selected item on the left side of the predator, which may cause it to turn away or towards the item.

To feed ASIMOV's predator, choose the desired prey or drug type from the Feeding-Choice drop-down menu, then click on the Feed button. This will spawn the selected item right at its mouth, effectively feeding it the item.
...
What happens to approach-avoidance decision when the predator is not hungry? What happens to decision about the noxious Flab prey when the predator is very hungry?
...
## A CHALLENGE!
Here we pose a challenge to the user: can you incorporate homeostatic plasticity into the pain system, just as we did with Reward State and the Homeostatic Reward Circuit? What would be the consequences? How would consumption of certain types of drug, say heroin, which directly suppresses the pain system, and cocaine, which does not, affect Reward State? Would you expect heroin to produce a more severe and painful withdrawal than cocaine?
